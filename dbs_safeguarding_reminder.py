...
from datetime import (
    date,
    timedelta,
)  # Date utilities for certificate expiry calculations
...
from google.oauth2.credentials import (
    Credentials,
)  # OAuth 2.0 credential handling for Google APIs
...
logging.info("Starting %s", CODENAME)
...
def retrieve_members_info() -> tuple[list, list]:
    ...
    """
    logging.info("[retrieve_members_info] Retrieving members information")
    ...
    while True:
        ...
        try:
            ...
            logging.error(
                "[retrieving_members_info] HTTP request failed on page: %s", page
            )
            sys.exit(1)
...
def expiring_within_one_month(members: List[Dict], cert_type: str, days=30) -> list:
    ...
    logging.info(
        "[expiring_within_one_month] Filtering the members with expiring "
        + cert_type
        + " certs"
    )
...
def expiring_to_excel_bytes(
    rows: List[Dict], sheet_name: str = "Expiring Certificates"
) -> bytes:
    """
    ...
    if not rows:
        logging.info("[expiring_to_csv_bytes] No list is found.")
...
def gmail_service_from_refresh_token():
    ...
    logging.info("[gmail_service_from_refresh_token] Refreshing gmail service token")
...
def send_email_with_excel(
    ...
    for filename, file_bytes, mime_type in attachments:
        ...
        msg.add_attachment(
            file_bytes, maintype=maintype, subtype=subtype, filename=filename
        )
    ...
    try:
        ...
        logging.exception("[send_email_with_excel] Failed to send email")
        raise
